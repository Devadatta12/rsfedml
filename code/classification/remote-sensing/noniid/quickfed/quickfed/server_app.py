...
from flwr.server.client_proxy import ClientProxy
from quickfed.task import load_model
...
def quick_select(results: list[tuple[ClientProxy, FitRes]]) -> NDArrays:
    """Compute in-place weighted average."""
    random_index = random.randint(0, len(results) - 1)
    params = [
        x for x in parameters_to_ndarrays(results[random_index][1].parameters)
    ]
...
class AggregateCustomMetricStrategy(FedAvg):
    ...
    def aggregate_fit(self, server_round: int, results: list[tuple[ClientProxy, FitRes]],
        ...
        if server_round%len(results) != 0:
            aggregated_ndarrays = quick_select(results)
        elif self.inplace:
